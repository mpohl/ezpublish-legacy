...
{literal}
    <script type="text/javascript">
        //$.cookie.json = true;
        if($.cookie('relationPosition')){
            var $relAttr = $.cookie('relationPosition');
            window.scrollTo(0,$('#'+$relAttr).offset().top-60);
            $.removeCookie('relationPosition',{ path: '/' });
        }
        var $relSubmits = $('.ezcca-edit-datatype-ezobjectrelationlist input:submit:not(.hide)');

        $relSubmits.on('click',function(event){
            var currentAttr = $(this).parents('.ezcca-edit-datatype-ezobjectrelationlist');
            $.cookie('relationPosition', currentAttr.attr('id'),{ path: '/' });
        });


       /*sortable tables objectrelationlist order*/
      var sortTable = $('.ezcca-edit-datatype-ezobjectrelationlist table.list');
         sortTable.sortable({
            ...
            onDrop: function  (item, container, _super) {
               var elements = container.el.find('[name^=ContentObjectAttribute_priority]');
               jQuery.each(elements,function(index, element){
                  $(element).val(index+1);
               });

               var action= '{/literal}{concat( 'ezautosave::savedraftpreview::', $object.id, '::', $edit_version, '::', $edit_language, '?ContentType=javascript' )|ezurl}{literal}'
               $.ez( 'ezautosave::savedraftpreview::{/literal}{$object.id}{literal}::{/literal}{$edit_version}{literal}::{/literal}{$edit_language}{literal}',
               {},
               function( data )
               {
                  if ( data.error_text ) {
